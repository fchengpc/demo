...
const Homepage = ({ doc, menu, lang, preview }) => {
    ...
    return <Fragment>{JSON.stringify(data)}</Fragment>;
};
...
export async function getStaticProps() {
    const repoId = "onelasttime";
    let schema = await fetch(`https://${repoId}.cdn.prismic.io/api`)
        .then((r) => r.json())
        .then((data) => {
            const ref = data.refs.find((r) => r.id === "master");
    ...
    // const schema = await Promise.resolve(getIntrospectionQueryResultData(
