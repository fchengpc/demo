...
async function getIntrospectionQueryResultData(apiEndpoint, graphql) {
    return getRef({ apiEndpoint })
        .then((ref) => {
            return fetch(
                `${graphql}?query=%7B%20__schema%20%7B%20types%20%7B%20kind%20name%20possibleTypes%20%7B%20name%20%7D%20%7D%20%7D%20%7D`,
                {
                    headers: {
                        "prismic-ref": ref,
                    },
...
export async function getStaticProps({
    ...
    const schema = await getIntrospectionQueryResultData(
        ...
        "https://onelasttime.cdn.prismic.io/api/v2"
    );
    const fragmentMatcher = new IntrospectionFragmentMatcher({
        introspectionQueryResultData: { __schema { types { kind name possibleTypes { name } } } },
    });
